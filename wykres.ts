        ...
        if (this.readyState == 4 && this.status == 200) {
            ...
            for (let i = 0; i < ob.length; i++) {
                ...
                area.addEventListener("click", function () {
                    ...
                    let ill = document.getElementById("ill") as HTMLButtonElement
                    let none = document.getElementById("none") as HTMLButtonElement
                    let cancel = document.getElementById("cancel") as HTMLButtonElement
                    save.onclick = function () {
                        let temperature:number  = parseFloat((<HTMLInputElement>document.getElementById("number")).value)
...
function setData(t: number, id: string) {
    var xhttp = new XMLHttpRequest();
    xhttp.open("POST", "ajax.php", true);
    xhttp.onreadystatechange = function () {
        if (this.readyState == 4 && this.status == 200) {
            ...
            w = id.substring(0, x)
            h = id.substring(x + 1)
            let src:string = "wykres.php?w=" + String(w) + "&h=" + String(h) + "&id=" + String(Math.random())
            img.src = src
            generateMap()
        }
    };
